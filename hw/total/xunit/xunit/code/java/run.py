...
def getCodeStyleResult(JavaFile):
	p = subprocess.Popen('java -jar /usr/share/java/checkstyle-7.0-all.jar -c /google_checks.xml %s'%(JavaFile), shell=True,
		         stdout=subprocess.PIPE,
		         stderr=subprocess.PIPE)
...
def getUnitTestScore(filename):
	p = subprocess.Popen('javac %s.java && java %s'%(filename, filename), shell=True,
                 stdout=subprocess.PIPE,
                 stderr=subprocess.PIPE)
